...
/* ─────────────────────────────────────────────
   ...
   한 학교 몫으로 가입한 계정이 다른 학교에서 쓰이는 것을 막기 위한 값들입니다.
   학교마다 사정이 다르므로 계정별로 users 탭에서 따로 조정할 수 있고,
   비어 있으면 아래 기본값이 적용됩니다.
   ───────────────────────────────────────────── */

const LIC = {
  SEATS: 2,          // 계정 하나가 쓸 수 있는 기기 수 (교실 PC + 개인 노트북)
  DAYS: 365,         // 이용 기간 기본값
  SELF_RELEASE: 1,   // 본인이 스스로 기기를 뺄 수 있는 횟수 (30일 기준)
  WARN_DEVICES: 3    // 최근 7일 안에 이 수를 넘겨 기기가 바뀌면 관리자 화면에 경고
};
...
const HEADERS = {
  ...
  devices:    ['소유자', '기기ID', '기기별명', '브라우저', '등록일', '최근접속', '상태'],
  loginlog:   ['시각', '아이디', '기기ID', '학교코드', '결과', '비고']
};
